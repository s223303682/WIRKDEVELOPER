...
namespace WIRKDEVELOPER.Controllers
{
    public class NurseController : Controller
    {
        ...
        {
            var booking = await Context.bookingNewPatients.Include(c => c.BookingNewPatientName).Where(x => x.BookingNewPatientID == i).FirstOrDefaultAsync();
            if (booking == null)
            {
            ...
            {
                BookingNewPatient = booking,
                BookingNewPatientID = booking.BookingNewPatientID,
            };
        ...
        {
            var x = await Context.admission.Include(c => c.BookingNewPatient).Include(c => c.BookingNewPatient.Patient).Where(c => c.AdmissionID == i).FirstOrDefaultAsync();
            if (x == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> Discharge(Discharge a)
        {
            if (ModelState.IsValid)
            {
                ...
                await Context.SaveChangesAsync();
                var b = await Context.bookings.Where(c => c.BookingID == ad.BookingNewPatientID).FirstOrDefaultAsync();
                b.Status = "Discharged";
        ...
        public IActionResult PrescribedPrescriptions()
        {
            ...
            var ad = Context.admission.Include(c => c.BookingNewPatient).Include(c => c.Bed).Include(c => c.Bed.Ward).Include(c => c.BookingNewPatient.Patient).Where(c => c.NurseId == HttpContext.Session.GetInt32("userint") && c.Status == "Admitted").AsEnumerable();
            var m = (from p in pres
                     join a in ad on p.PatientID equals a.BookingNewPatient.PatientID
                     select new PresAd
                     {
                         PatientName = $"{a.BookingNewPatient.Patient.PatientName} {a.BookingNewPatient.Patient.PatientSurname} ",
                         PatientID = a.BookingNewPatient.Patient.PatientIDNO,
                         Bed = $"{a.Bed.BedNumber}",
        ...
        {
            return View(Context.bookingNewPatients.Include(c => c.Patient).Include(c => c.OperationTheatre).Include(c => c.Anaesthesiologist.ApplicationUser).Include(c => c.Surgeon.ApplicationUser).Where(c => c.Status == "Booked"));
        }
        public async Task<IActionResult> Admit(int i, int pat)
        {
            var booking = await Context.bookingNewPatients.Include(c => c.Patient).Where(x => x.BookingNewPatientID == i && x.PatientID == pat).FirstOrDefaultAsync();
            if (booking == null)
            {
                TempData["e"] = "Booking not found!";
                return RedirectToAction(nameof(Bookings));
        ...
        {
            return View(Context.admission.Include(c => c.BookingNewPatient.Patient).Include(c => c.Bed).Include(c => c.Bed.Ward).Include(c => c.BookingNewPatient.OperationTheatre).Include(c => c.BookingNewPatient.Surgeon).Include(c => c.BookingNewPatient.Surgeon.ApplicationUser).Include(c => c.BookingNewPatient.Anaesthesiologist.ApplicationUser).Where(c => c.Status == "Admitted" && c.NurseId == HttpContext.Session.GetInt32("userint")));
        }
        ...
        {
            return View(Context.medicationPrescriptions.Include(c => c.PharmacyMedication));
        }
        ...
        public async Task<IActionResult> AdministerMeds([FromBody] List<MedsID> meds)
        {
            if (meds.Any())
            {
                ...
                {
                    var x = await Context.medicationPrescriptions.Where(c => c.MedicationPrescriptionID == item.MedId).FirstOrDefaultAsync();
                    if (x != null)
                    {
                        x.QuantityGiven = x.QuantityGiven + item.Quantity;
                        Context.medicationPrescriptions.Update(x);
                        await Context.SaveChangesAsync();
                    }
                }
                return Json(true);
        ...
        public async Task<IActionResult> AdmitPatient(Admission admission)
        {
            if (ModelState.IsValid)
            {
                ...
                await Context.SaveChangesAsync();
                var x = Context.bookingNewPatients.Where(c => c.BookingNewPatientID == admission.BookingNewPatientID).FirstOrDefault();
                x.Status = "Admitted";
                Context.bookingNewPatients.Update(x);
                await Context.SaveChangesAsync();
                TempData["m"] = "Patient has been admitted!";
                return RedirectToAction(nameof(AdmittedPatients));
            }
            TempData["m"] = "Something went wrong. Please try again";
            return RedirectToAction(nameof(Admit), new { id = admission.BookingNewPatientID, pat = admission.Pat });
        }
        [HttpPost]
        ...
        public async Task<IActionResult> UpdatePatientVitalsE([FromBody] List<PatientVitals> vitals)
        {
            if (ModelState.IsValid)
            {
                var pat = await Context.bookingNewPatients.Include(c => c.Surgeon).Include(c => c.Surgeon.ApplicationUser).Where(c => c.BookingNewPatientID == vitals[0].BookingNewPatientID).FirstOrDefaultAsync();
